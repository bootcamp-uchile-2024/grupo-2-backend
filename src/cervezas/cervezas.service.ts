...
export class CervezasService {
  ...
  constructor(
    ...
    @InjectRepository(TipoCerveza)
    private readonly TipoCervezaRepository: Repository<TipoCerveza>,
    @InjectRepository(Proveedor)
    private readonly ProveedorRepository: Repository<Proveedor>,
    @InjectRepository(Amargor)
    private readonly AmargorRepository: Repository<Amargor>,
  ) {}
  ...
  async create(createCervezaDto: CreateCervezaDto) {
    ...
    if (!existe) {
      ...
      const tipo_id = await this.TipoCervezaRepository.findOneBy({
        nombre: createCervezaDto.tipo_cerveza,
      });
      entidad.id_tipo = tipo_id.id;
      const amargor = await this.AmargorRepository.findOneBy({
        nivel: createCervezaDto.amargor,
      });
      entidad.id_amargor = amargor.id;
      const proveedor = await this.ProveedorRepository.findOne({
        where: {
          nombre: createCervezaDto.proveedor.nombre,
      ...
      if (proveedor) {
        ...
        const guardar_proveedor = await this.ProveedorRepository.save(
          createCervezaDto.proveedor,
        );
      ...
      throw new HttpException(
        'La cerveza que se intenta crear ya existe',
  ...
  async findAll(
    ...
    const resultadoDto = resultado.map((entidad) =>
      CervezaMapper.entityToDto(entidad),
    );
    return resultadoDto;
  }
  ...
  async update(id: number, updateCervezaDto: UpdateCervezaDto) {
    ...
    if (existe) {
      ...
      const tipo_id = await this.TipoCervezaRepository.findOneBy({
        nombre: updateCervezaDto.tipo_cerveza,
      });
      console.log(entidad);
      entidad.id_tipo = tipo_id.id;
      const amargor = await this.AmargorRepository.findOneBy({
        nivel: updateCervezaDto.amargor,
      });
      entidad.id_amargor = amargor.id;
      const proveedor = await this.ProveedorRepository.findOne({
        where: {
          nombre: Like(updateCervezaDto.proveedor.nombre),
      ...
      throw new HttpException(
        'La cerveza que se intenta actualizar no existe',
        HttpStatus.BAD_REQUEST,
      );
    }
  }
  ...
  async remove(id: number) {
    ...
    if (existe) {
      ...
      throw new HttpException(
        'La cerveza que intenta eliminar no existe',
        HttpStatus.BAD_REQUEST,
      );
    }
  }

  async cargarImagenCerveza(
    id: number,
    ...
    if (cerveza) {
      ...
      try {
        try {
          ...
          await rm(ruta);
          console.log(`El archivo en "${ruta}" ha sido eliminado.`);
        } catch (error) {
          if (error.code === 'ENOENT') {
            console.log(`El archivo en "${ruta}" no existe.`);
          } else {
        ...
        throw new HttpException(
          `La imagen no pudo ser actualizadaa,${error}`,
          HttpStatus.BAD_REQUEST,
        );
      }
    } else {
      throw new HttpException(
        'El id de cerveza ingresado no existe',
        HttpStatus.BAD_REQUEST,
      );
    }
    return '';
  ...
  async actualizarEstado(id: number, estado: estado) {
    ...
    if (existe) {
      const guardar_cerveza = await this.CervezaRepository.update(id, {
  ...
  async actualizarStock(id: number, cantidad: number) {
    ...
    if (existe) {
      ...
      if (nuevo_stock >= 0) {
        ...
        throw new HttpException(
          'La cerveza que se intenta actualizar no tiene stock suficiente',
          HttpStatus.BAD_REQUEST,
        );
      }
    } else {
