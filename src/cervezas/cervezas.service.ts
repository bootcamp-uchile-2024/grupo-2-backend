...
export class CervezasService {
  ...
  async findAll(nombre: string, marca, categoria, descripcion, precio, proveedor, region, comuna, amargor, graduacion, formato): Promise <CreateCervezaDto[]> {
    const instruccionSQL = "SELECT nombre, descripcion FROM Cerveza;";
    const resultadoQuery = await this.DataSource.query(instruccionSQL);
